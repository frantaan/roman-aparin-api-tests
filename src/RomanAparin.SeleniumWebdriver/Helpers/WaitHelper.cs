...
using NotFoundException = RomanAparin.SeleniumWebDriver.Exсeptions.NotFoundException;
...
namespace RomanAparin.SeleniumWebDriver.Helpers
{
    public static class WaitHelper
    {
        ...
        public static T DoRetryWithReturn<T>(Func<T> func, int retries = 0)
        {
        ...
        public static T WaitForCondition<T>(Func<T> func, Func<T, bool> condition, int retries = 0)
        {
        ...
        public static void DoRetry(Action action, int retries = 0)
        {
            var waiteAndRetryPolicy = HandleExceptionPolicy(GetRetries(retries), TimeBetweenRetries)
